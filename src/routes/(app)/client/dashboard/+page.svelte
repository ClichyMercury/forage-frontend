...
<svelte:head><title>{$t('dashboard.client.title')}</title></svelte:head>
...
<div class="mb-6 flex items-start lg:items-center justify-between flex-wrap gap-3">
  <div class="min-w-0 flex-1">
    <h2 class="font-display font-black text-2xl lg:text-3xl leading-tight wrap-break-word" style="color: #0f1f5c; letter-spacing: -0.02em">
      {$t('dashboard.client.hello', { name: auth.user?.fullName?.split(' ')[0] ?? '' })}
    </h2>
    <p class="text-sm text-slate-500 mt-1">{$t('dashboard.client.subtitle')}</p>
  </div>
    ...
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1">
        <p class="font-display font-bold text-base" style="color: #0f1f5c">
          {demandesUrgentes.length === 1
            ? $t('dashboard.client.offer_waiting_one')
            : $t('dashboard.client.offer_waiting_many', { count: demandesUrgentes.length })}
        </p>
        <p class="text-xs mt-1 text-slate-500">{$t('dashboard.client.offer_waiting_sub')}</p>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
  <StatCard title={$t('dashboard.client.stat_total')}       value={stats().total}       icon="folder_open"    color="blue" />
  <StatCard title={$t('dashboard.client.stat_pending')}     value={stats().enAttente}   icon="hourglass_empty" color="orange" />
  <StatCard title={$t('dashboard.client.stat_in_progress')} value={stats().enCours}     icon="autorenew"      color="purple" />
  <StatCard
    title={$t('dashboard.client.stat_offers')}
    value={stats().offresRecues}
    icon="mark_email_read"
    color="cyan"
    subtitle={stats().aTraiter > 0
      ? `${stats().aTraiter} ${$t('dashboard.client.stat_to_process')}`
      : $t('dashboard.client.stat_all_processed')}
  />
</div>
<div class="grid grid-cols-3 gap-3 mb-6">
  <StatCard title={$t('dashboard.client.stat_accepted')} value={stats().acceptees} icon="check_circle" color="green" />
  <StatCard title={$t('dashboard.client.stat_refused')}  value={stats().refusees}  icon="cancel"       color="red" />
  <StatCard title={$t('dashboard.client.stat_closed')}   value={stats().cloturees} icon="lock"         color="blue" />
</div>

<div class="bg-white rounded-2xl border border-slate-100">
  <div class="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
    <h3 class="font-display font-bold text-slate-900">{$t('dashboard.client.history')}</h3>
    <a href="/client/demandes" class="text-sm text-brand-600 hover:text-brand-700 font-semibold">{$t('common.see_all')}</a>
  </div>
  ...
  {:else if demandes.length === 0}
    <div class="py-14 text-center px-6">
      ...
      <p class="text-slate-700 font-semibold text-sm">{$t('dashboard.client.no_demandes')}</p>
      <p class="text-slate-400 text-xs mt-1 mb-4">{$t('dashboard.client.no_demandes_sub')}</p>
      <button onclick={() => goto('/client/demandes/new')}
  ...
  {:else}
    <div class="hidden lg:grid grid-cols-12 gap-3 px-5 py-2.5 bg-slate-50 border-b border-slate-100 text-xs font-semibold text-slate-400 uppercase tracking-wide">
      <div class="col-span-5">{$t('dashboard.client.col_location')}</div>
      <div class="col-span-2">{$t('dashboard.client.col_type')}</div>
      <div class="col-span-2">{$t('dashboard.client.col_date')}</div>
      <div class="col-span-3">{$t('dashboard.client.col_status')}</div>
    </div>
